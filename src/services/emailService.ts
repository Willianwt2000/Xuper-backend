...
export const sendVerificationCodeEmail = async (
  ...
  try {
    ...
    console.log(`📧 Usando remitente: ${process.env.EMAIL_FROM}`);

    const response = await apiInstance.sendTransacEmail({
      sender: { email: process.env.EMAIL_FROM!, name: "Xuper TV" },
      to: [{ email: to }],
                  ...
                  <tr>
                    <td style="padding: 40px 40px 20px 40px; text-align: center;">
                      <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: 800; letter-spacing: 1px;">
                        XUPER <span style="color: #00F0FF;">APP</span>
                      </h1>
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td style="background-color: #0f0f0f; padding: 20px; text-align: center; border-top: 1px solid #222;">
                      ...
                      <p style="color: #444444; font-size: 12px; margin-top: 10px;">
                        © ${new Date().getFullYear()} XuperApp Inc.
                      </p>
                    </td>
                  </tr>
                </table>
    ...
    console.error("❌ Error detallado enviando el correo:");
